...
**Supersedes:** DARM-ANN v6.3 (May 2026)
...
|**v6.4**|**May 2026**|**Context Bridge Layer (§25); DACS integration + KV Prefix Sharing (§26); Context Engineering Architecture Summary (§27); 16-layer stack update; P58–P61; A23–A26; refs [62]–[66]**|

-----

## What’s New in v6.4

- **§25 (new):** Context Bridge Layer — bidirectional formal protocol between WM context window and blockchain tiers; four bridge flows (downward assembly, upward commit, cross-session serialization, cross-agent handoff)
- **§26 (new):** DACS integration into RCN (90–98.4% steering accuracy, arXiv:2604.07911); KV Prefix Sharing Protocol via Branch DAG (30% inference compute reduction on DCoT chains)
- **§27 (new):** Context Engineering Architecture Summary — unified flow diagram, operation-to-flow mapping table, comparison of DARM-ANN Context Bridge vs. browser-layer context tools
- **Context Assembly Engine (CAE):** Six-slot priority-weighted WM assembly from RRC/STM/EB/LTM with token budget enforcement
- **Context Budget Manager (CBM):** Per-tier token budget table; compression priority order
- **Anchor Token Injector (ATI):** Prevents omission constraint decay by re-injecting LTM constraints into WM at rate λ_atf proportional to context depth
- **Cross-session serialization:** Session boundary blocks committed to LTM via HLC; cold-start recovery via CAE from session anchor
- **Cross-agent context handoff:** Flow D formalizes DCoT step context transfer including provenance hash and constraint carry-forward
- **P58–P61:** Four new formal proofs — CAE optimality, DACS steering accuracy, KV prefix sharing compute reduction, anchor token injection constraint recall bound
- **A23–A26:** Four new assumptions for CAE MI ranking, DACS focus completeness, Branch DAG prefix correctness, ATI tuning feasibility
- **References [62]–[66]:** DACS paper (arXiv:2604.07911), Gamage 2026 omission constraints, Mem0 LoCoMo 2026, Google Context Engineering (2025), KV cache optimization guide
...
1. [Context Bridge Layer](#25-context-bridge-layer)
1. [DACS Integration and KV Prefix Sharing](#26-dacs-integration-and-kv-prefix-sharing)
1. [Context Engineering Architecture Summary](#27-context-engineering-architecture-summary)
1. [High-Entropy Neural Data Structures — Mathematical Treatment](#28-high-entropy-neural-data-structures--mathematical-treatment)
1. [Power and Energy Mathematics](#29-power-and-energy-mathematics)
1. [Risk Analysis](#30-risk-analysis)
1. [Further Research](#31-further-research)
1. [Assumption Registry](#32-assumption-registry)
1. [Proof Dependency Graph](#33-proof-dependency-graph)
1. [Implementation Roadmap](#34-implementation-roadmap)
1. [Conclusion](#35-conclusion)

- [References](#references)
- [Appendix A: Glossary](#appendix-a-glossary)
- [Appendix B: Proof Index (P1–P61)](#appendix-b-proof-index)
...
## 25. Context Bridge Layer

*New in v6.4. Formalizes the bidirectional protocol connecting the Working Memory context window to the local branch DAG and global blockchain memory tiers.*

### 25.1 The Fundamental Problem: WM is RAM, Not Storage
...
A 2026 study across 4,416 trials at six conversation depths confirmed that commission constraints hold near 100% while omission constraints decay with depth — this is a memory architecture problem, not a model problem.

DARM-ANN v6.3 specified five memory tiers but left one critical gap: the formal protocol governing what enters and leaves the context window (WM) at inference time, how it is assembled from the blockchain tiers below, and how WM outputs are committed back. The Context Bridge Layer closes this gap.

**Core principle:** The context window (WM tier) is transient RAM. The blockchain (LTM) is permanent tamper-evident storage. The Context Bridge is the bidirectional protocol mediating between them across the full five-tier stack.

### 25.2 Four Bridge Flow Types

The Context Bridge specifies four distinct information flows:

```
┌─────────────────────────────────────────────────────────────────────┐
│                    CONTEXT BRIDGE LAYER                              │
│                                                                      │
│  Flow A: Downward Assembly  LTM/RRC/STM/EB → WM (inference start)  │
│  Flow B: Upward Commit      WM outputs → EB → STM → LTM            │
│  Flow C: Cross-session      WM state → LTM anchor at session end    │
│  Flow D: Cross-agent        Agent_i WM summary → Agent_j WM (DCoT) │
│                                                                      │
│  Mechanisms:                                                         │
│  • Context Assembly Engine (CAE) — governs Flow A                   │
│  • Context Budget Manager (CBM) — enforces token limits all flows   │
│  • Anchor Token Injector (ATI) — LTM constraints → WM persistence   │
│  • KV Prefix Share Protocol (KVPSP) — cross-agent KV reuse         │
│  • DACS — context isolation preventing cross-agent pollution        │
└─────────────────────────────────────────────────────────────────────┘
```

### 25.3 Flow A — Context Assembly Engine (CAE)

At inference start, the CAE constructs WM content from lower tiers using six context types grounded in Google’s 2025 context engineering taxonomy:

```
ContextTypes := {
  instructions:    system prompt, task specification, constraints
  anchor_tokens:   high-salience LTM facts (re-injected via ATI)
  agent_state:     current task, step_id, ExecutionContext ref
  background:      RRC retrieval (P43: O(1) avg)
  conversation:    EB ring buffer — recent reasoning traces
  tool_outputs:    last MCP Gateway tool call results
}
```

**CAE priority-weighted budget allocation:**

```
Algorithm CAE(agent_a, task_t, W_budget_tokens):
  slots = {instructions: 0.15, anchor_tokens: 0.10, agent_state: 0.10,
           background: 0.40, conversation: 0.20, tool_outputs: 0.05}
  
  wm.instructions  = STM.fetch(agent_a.specialization_prompt)
  wm.anchor_tokens = ATI.get_anchors(agent_a, task_t)
  wm.agent_state   = ExecContext.serialize(task_t)
  wm.background    = RRC.query(task_t.embedding, k=top_k)
  wm.conversation  = EB.tail(agent_a, n_tokens=slots.conversation × W_budget)
  wm.tool_outputs  = MCP_log.last(agent_a, n_tokens=slots.tool_outputs × W_budget)
  
  for slot, content in wm:
    if len(content) > slots[slot] × W_budget:
      wm[slot] = CBM.compress(content, target=slots[slot] × W_budget)
  
  return assemble_prompt(wm)
```

**P58 — Context Assembly Optimality:**

*Claim:* The CAE priority ordering maximizes expected task completion quality Q(wm) per token budget W_budget.

*Proof sketch:* Decompose Q(wm) = Σ_s w_s × MI(slot_s; task_output). Instructions and anchor tokens have highest MI — they encode task specification and hard constraints that gate output correctness. Background (RRC) has high MI for factual tasks. Conversation has bounded MI (diminishing returns beyond k turns). Tool outputs are conditionally useful. The priority ordering follows empirical MI ranking: Mem0’s 2026 LoCoMo benchmark showed hierarchical extraction outperformed full-context by 18.7 percentage points while maintaining a 4× reduction in token count. Requires A23. **QED sketch.**

### 25.4 Context Budget Manager (CBM)

|Model Tier        |W_budget     |KV Cache Budget|
|------------------|-------------|---------------|
|TinyLM (0.5–2B)   |3,072 tokens |512MB          |
|SLM-Small (3–7B)  |6,144 tokens |2GB            |
|SLM-Medium (7–13B)|24,576 tokens|8GB            |
|SLM-Large (13–70B)|96,000 tokens|32GB           |

CBM compression priority: trim EB traces first, summarize background second, truncate tool outputs third. Never compress: instructions, anchor tokens, agent_state — compressing these causes correctness failures.

### 25.5 Anchor Token Injector (ATI)

Context management for LLM agents has been studied at three levels of granularity: single-agent memory across turns, single-agent tool/retrieval bloat, and multi-agent orchestration. The omission constraint decay problem — constraints followed at turn 1 violated at turn 10 — occurs at all three levels and is structural, not model-dependent.

The ATI prevents decay by re-injecting high-salience LTM constraints with injection priority that *increases* with step depth — directly counteracting the decay process:

```
Algorithm ATI(agent_a, step_i, wm):
  anchors = LTM.query(filter={type:"constraint", agent:agent_a})
  for a in anchors:
    a.injection_priority = a.salience × (1 - exp(-λ_atf × step_i))
  top_anchors = sort_by(anchors, injection_priority)[:k_anchors]
  wm.anchor_tokens = serialize(top_anchors)
  return wm
```

**P61 — Anchor Token Injection Prevents Omission Constraint Decay:**

*Claim:* With ATI injection at frequency λ_atf, constraint recall at step D ≥ 1 - exp(-λ_atf × D × w_anchor/W_budget).

*Proof:* Attention weight on a token decays approximately exponentially with intervening tokens. Each re-injection resets the decay clock. Expected injections by step D = λ_atf × D. Minimum recall is bounded by the formula above. Requires A26.

**ATI + blockchain integration:** Constraint records in LTM carry the highest salience scores, are retrograde-protected (P44, κ=1.2), and are consensus-committed. The ATI reads from LTM — so even constraints introduced post-deployment are automatically picked up at the next injection cycle. The blockchain is the canonical source of active constraints.

### 25.6 Flow B — Upward Commit

Already specified in §4 (Memory Formation Pipeline) and §5 (CDCP). The Context Bridge formalizes the W_class gate:

```
UpwardCommit(output): EB.write() always
  if salience ≥ θ_stm: STM.write(W_class=local_write)  // OER
  if salience ≥ θ_ltm: CDCP.propose(W_class=global_write)  // HLC
```

### 25.7 Flow C — Cross-Session Serialization to LTM

Entirely absent from v6.3. Answers: what persists across session boundaries?

```
SessionEnd(agent_a, session_id):
  session_anchor = {session_id, agent_id, task_context,
                    key_facts=[e∈EB if e.salience≥θ_anchor],
                    constraint_set=wm.anchor_tokens,
                    hash=blake3(concat(above_fields))}
  LTM.commit(session_anchor, block_type="session_boundary")  // HLC path

SessionStart(agent_a):
  anchor = LTM.query(filter={agent:agent_a, type:"session_boundary"},
                     sort=desc_timestamp, limit=1)
  if anchor: EB.preload(anchor.key_facts); return CAE(agent_a, anchor.task_context)
  else:      return CAE(agent_a, new_task)
```

Session anchors are global_write (HLC path) — tamper-evident, BFT consensus-committed. No agent can falsify its prior session context.

### 25.8 Flow D — Cross-Agent Context Handoff

Formalizes DCoT step context transfer with provenance:

```
ContextHandoff(agent_src, agent_dst, step_result):
  handoff = {result: step_result,
             reasoning: EB.last_n(agent_src, n=5),
             constraints: ATI.get_anchors(agent_src, task),
             provenance: LTM.hash(step_result)}
  EB.write(handoff, agent=agent_dst, W_class=local_write)
```

Each DCoT agent inherits the previous step’s result AND the active constraint set. Constraint decay across agent boundaries is prevented because constraints carry over via the handoff, not just via the new agent’s fresh context window.

-----

## 26. DACS Integration and KV Prefix Sharing

*New in v6.4.*

### 26.1 DACS — Solving Context Pollution in the RCN

Multi-agent LLM orchestration systems suffer from context pollution: when N concurrent agents compete for the orchestrator’s context window, each agent’s task state, partial outputs, and pending questions contaminate the steering interactions of every other agent, degrading decision quality.

Context isolation is agent-triggered, asymmetric, and deterministic: the context window contains exactly F(a_i) + R_{-i} during steering, eliminating cross-agent contamination without requiring context compression or retrieval.

**Paper:** Patel, N. (2026). *Dynamic Attentional Context Scoping*. arXiv:2604.07911.

DACS maps directly onto the RCN’s existing architecture. The RCN already holds two context views:

- **Registry entry (≤200 tokens):** A2A Agent Card — already in §23.2
- **Full context:** ExecutionContext stack — already in §14.2

DACS formalizes the switch protocol:

```
REGISTRY mode: hold Agent Cards for all N agents (≤200 tokens × N)
FOCUS(a_i) mode: F(a_i) = CAE(a_i, task_t, W_budget)
                 R_{-i} = {AgentCard(a_j) | j≠i}
                 WM_RCN = F(a_i) ∪ R_{-i}

Switch REGISTRY → FOCUS(a_i):
  agent_a_i emits STEERINGREQUEST
  OR a_i.confidence < threshold
  OR a_i triggers V/P pattern

Switch FOCUS(a_i) → REGISTRY:
  steering resolved OR timeout T_focus (default 30s)
```

For N=10, W_budget=6,144 (SLM-Small): WM_RCN = 6,144 + 1,800 = 7,944 tokens — within budget.

**DACS and blockchain grounding:** FOCUS mode context is pulled from the Branch DAG. Every fact in F(a_i) is either a CDCP-committed LTM record, a salience-gated STM entry, or a branch DAG OER write. Context pollution from fabricated agent states is structurally prevented — the source of truth is the cryptographically chained branch DAG.

**P59 — DACS Steering Accuracy:** Across all 8 synthetic scenarios, DACS achieves 90.0–98.4% steering accuracy versus 21.0–60.0% for a flat-context baseline (p < 0.0001 throughout), with wrong-agent contamination falling from 28–57% to 0–14% and context efficiency ratios of up to 3.53×. Source: Patel (2026) arXiv:2604.07911. Requires A24.

### 26.2 KV Prefix Sharing Protocol (KVPSP)

The KV cache stores per-layer key and value tensors for every token in the context window so that subsequent tokens can attend to them without recomputing. Memory grows linearly with context length, layer count, and head dimension.

For DCoT chains, agents sharing task ancestry also share a large common context prefix. KVPSP uses the Branch DAG’s ancestry structure to compute the prefix once and share it:

```
KVPSP(agent_a, agent_b, task_t):
  ancestor = Branch_DAG.common_ancestor(a.step_id, b.step_id)
  shared_prefix = CAE_prefix(ancestor, W_budget)
  KV_shared = compute_kv(shared_prefix)
  Branch_DAG.write(hash(shared_prefix), KV_shared,
                   W_class=local_write, ttl=T_occ×3)
  agent_a.kv_cache = KV_shared + compute_kv(agent_a.unique_tokens)
  agent_b.kv_cache = KV_shared + compute_kv(agent_b.unique_tokens)
```

**P60 — KV Prefix Sharing Compute Reduction:**

*Claim:* For agents sharing d_shared of d_total context tokens, KVPSP reduces total KV compute by d_shared / (2 × d_total).

*Proof:* Without sharing: 2×d_total KV computations. With sharing: d_shared + 2×(d_total - d_shared) = 2×d_total - d_shared. Reduction fraction = d_shared / (2×d_total). For d_shared = 0.60×d_total: reduction = 30%. **QED.**

FP8 KV cache halves memory with sub-0.7 point accuracy regression — within noise for most production workloads — and the 50% memory savings translate to 30–50% throughput gains via larger batch size at the same VRAM budget.

Combined KVPSP (30%) + FP8 (50%): ~65% total KV memory reduction for DCoT workloads. TinyLM agents on Pi nodes handle substantially longer reasoning chains before triggering AI Gateway fallback.

**Assumption A25 — Branch DAG ancestry correctly identifies shared prefix tokens.** Validated by construction: CAE uses deterministic priority-weighted assembly, so identical ancestry → identical prefix tokens.

-----

## 27. Context Engineering Architecture Summary

*New in v6.4.*

### 27.1 Context Flow Diagram

```
                    ┌──────────────────────────────────────┐
                    │         CONTEXT BRIDGE LAYER          │
  ┌─────────────────┼──────────────────────────────────────┤
  │  INFERENCE →    │  CAE: 6-slot priority assembly        │
  │  CROSS-AGENT →  │  CBM: per-tier token budgets         │
  │  SESSION END →  │  ATI: LTM constraint re-injection     │
  │  KV SHARING →   │  KVPSP: Branch DAG prefix cache      │
  │  STEERING →     │  DACS: Registry/Focus mode switching  │
  └─────────────────┴──────────────────────────────────────┘
         ↑↓ Flow A (assembly)           ↑↓ Flow B/C/D (commit)
  ┌──────────────────────────────────────────────────────────┐
  │                FIVE-TIER MEMORY STACK                    │
  │  WM  [context window → KV cache] ← CAE fills            │
  │  EB  [session ring buffer]       ← Flow B/D writes       │
  │  STM [node-local Redis]          ← salience gate         │
  │  LTM [blockchain; Tendermint]    ← CDCP/HLC             │
  │  RRC [hot LSH cache]             ← feeds CAE background  │
  └──────────────────────────────────────────────────────────┘
```

### 27.2 Context Flow by Operation

|Operation          |Bridge Flow           |HCP Path    |Blockchain Touch |
|-------------------|----------------------|------------|-----------------|
|Inference start    |Flow A: CAE           |read_only   |RRC/STM/LTM read |
|Anchor injection   |ATI re-inject         |read_only   |LTM read         |
|Reasoning output   |Flow B: EB write      |local_write |Branch DAG       |
|Significant finding|Flow B: CDCP          |global_write|LTM commit       |
|Session end        |Flow C: session anchor|global_write|LTM session block|
|DCoT handoff       |Flow D: cross-agent EB|local_write |Branch DAG       |
|RCN steering       |DACS FOCUS            |read_only   |Branch DAG read  |
|KV prefix          |KVPSP branch lookup   |read_only   |Branch DAG read  |

### 27.3 DARM-ANN as Universal Context Store

The storage of memories is done on a blockchain to provide a persistent, decentralized storage. The extension builds a lasting bridge between AI tools as a continuous context provider.

DARM-ANN formalizes and extends this pattern across every architectural dimension:

|Dimension             |Browser-layer context tools|DARM-ANN Context Bridge                 |
|----------------------|---------------------------|----------------------------------------|
|Persistence           |Centralized cloud DB       |BFT blockchain; f < N/3 Byzantine       |
|Tamper evidence       |None                       |Cryptographic hash chain (P14)          |
|Cross-agent           |Manual copy                |Flow D with provenance hash             |
|Constraint enforcement|User-managed               |ATI with salience decay prevention (P61)|
|Context assembly      |Manual prompting           |CAE priority-weighted 6-slot budget     |
|Cross-session         |Session-scoped DB          |LTM session boundary blocks via HLC     |
|Provenance            |None                       |Every context entry has LTM hash        |

The blockchain is not supplementary to context management in DARM-ANN — it IS the context store. The Context Bridge formalizes the interface.

### 27.4 Updated 16-Layer Stack (v6.4)

```
┌──────────────────────────────────────────────────────────────────────┐
│                      APPLICATION INTERFACE LAYER                      │
├──────────────────────────────────────────────────────────────────────┤
│         INTERACTION MODEL PROTOCOL STACK (A2A/ACP/OASF) [v6.3]      │
├──────────────────────────────────────────────────────────────────────┤
│         AI GATEWAY                                  [v6.3]           │
├──────────────────────────────────────────────────────────────────────┤
│         MCP GATEWAY                                 [v6.3]           │
├──────────────────────────────────────────────────────────────────────┤
│         ROUTER / CHAIN-MANAGER NODE (RCN)           [v6.2]          │
│         + DACS Registry/Focus context modes         [v6.4]          │
├──────────────────────────────────────────────────────────────────────┤
│         CONTEXT BRIDGE LAYER                        [NEW v6.4]       │
│  CAE · CBM · ATI · KVPSP · Flow A/B/C/D                             │
│  Bidirectional WM ↔ Blockchain protocol                              │
├──────────────────────────────────────────────────────────────────────┤
│         ESE + BVAS + GTE                            [v5.0]           │
├──────────────────────────────────────────────────────────────────────┤
│         RLRF + MARKOV GRAPH G_M                     [v4.0]           │
...
│         HIERARCHICAL SLM/TinyLM (AI Gateway-backed) [v3.0]          │
├──────────────────────────────────────────────────────────────────────┤
│         FIVE-TIER MEMORY (WM/EB/STM/LTM/RRC)       [v6.0]           │
│         Context Bridge interfaces all five tiers    [v6.4]          │
├──────────────────────────────────────────────────────────────────────┤
│         BLOCKCHAIN + OER + HLC                      [v3.0 + v6.2]   │
│         Context Bridge commits via OER/HLC          [v6.4]          │
├──────────────────────────────────────────────────────────────────────┤
│         DATA STRUCTURES (LSH, B-tree, G_M)          [v3.0]          │
├──────────────────────────────────────────────────────────────────────┤
│         NEUROMORPHIC SUBSTRATE                      [v6.1]           │
...
## 28. High-Entropy Neural Data Structures — Mathematical Treatment
...
## 29. Power and Energy Mathematics
...
|A22|Agent Cards updated within 1 heartbeat (T_heartbeat=5s) of state change                                        |A2A (§23.4)                |Measure card staleness under network partition                                            |
|A23|Context type MI ranking (instructions>anchors>state>background>conversation>tools) holds for ≥85% of task types|P58                        |Measure task quality by slot priority across 10,000 runs                                  |
|A24|DACS Focus ExecutionContext captures all RCN steering information                                              |P59                        |Measure DACS vs. flat-context steering accuracy; verify ≥90%                              |
|A25|Branch DAG ancestry correctly identifies shared context prefix tokens                                          |P60                        |Structural verification: deterministic CAE assembly                                       |
|A26|ATI injection frequency λ_atf can prevent omission constraint decay within token budget                        |P61                        |Measure omission constraint recall vs. λ_atf across 6 conversation depths                 |

-----

## 33. Proof Dependency Graph
...
**v6.4 Context Bridge and Engineering (NEW):**
31. CAE: WM content maximized per token budget via priority-weighted 6-slot assembly — P58
32. DACS: 90–98.4% RCN steering accuracy via Registry/Focus mode switching — P59
33. KVPSP: 30% inference compute reduction on DCoT chains via Branch DAG prefix sharing — P60
34. ATI: omission constraint decay prevented via salience-proportional LTM re-injection — P61
35. Four bridge flows (A/B/C/D) formally specify the complete WM ↔ blockchain protocol
36. Cross-session continuity via LTM session boundary blocks — tamper-evident, BFT consensus-committed
37. Cross-agent context handoff with provenance-hashed EB writes — constraint carry-forward across DCoT boundaries

The Context Bridge Layer completes the final formal gap: the blockchain is not a supplementary memory store — it IS the persistent context substrate. Every context token has cryptographic provenance. Every constraint is consensus-committed. Every session boundary is tamper-evident. Context management in DARM-ANN is a formally specified, proof-backed, blockchain-grounded protocol.
...
[61] Zylos AI Research. (2026). Agent Interoperability Protocols 2026. <https://zylos.ai/research/2026-03-26-agent-interoperability-protocols-mcp-a2a-acp-convergence>

[62] Patel, N. (2026). Dynamic Attentional Context Scoping: Agent-Triggered Focus Sessions for Isolated Per-Agent Steering in Multi-Agent LLM Orchestration. arXiv:2604.07911. <https://arxiv.org/abs/2604.07911>

[63] Gamage, Y. (2026). Omission Constraints Decay While Commission Constraints Persist in Long-Context LLM Agents. April 2026.

[64] Mem0 Research. (2026). Context Window Is RAM, Not Storage: Why Most Agent Failures Happen. LoCoMo Benchmark Results. <https://mem0.ai/blog/context-window-is-ram-not-storage-why-most-agent-failures-happen-how-to-fix-them-in-2026>

[65] Milam, K. & Gulli, A. (2025). Context Engineering: Sessions & Memory. Google. November 2025. <https://vanducng.dev/2026/01/12/Google-Context-Engineering-Sessions-Memory-Summary/>

[66] Digital Applied. (2026). KV Cache Optimization for LLMs 2026: Engineering Guide. <https://www.digitalapplied.com/blog/kv-cache-optimization-techniques-2026-engineering-guide>
...
|Context Bridge        |Bidirectional protocol mediating between WM context window and blockchain memory tiers                                                                      |
|CAE                   |Context Assembly Engine — six-slot priority-weighted assembly of WM content from RRC/STM/EB/LTM                                                             |
|CBM                   |Context Budget Manager — enforces per-tier token limits; compression priority order                                                                         |
|ATI                   |Anchor Token Injector — periodically re-injects high-salience LTM constraints into WM to prevent omission decay                                             |
|KVPSP                 |KV Prefix Sharing Protocol — uses Branch DAG ancestry to share KV cache prefix across DCoT chain agents                                                     |
|DACS                  |Dynamic Attentional Context Scoping — RCN operates in Registry mode (Agent Cards ≤200 tokens) or Focus mode (full agent context); prevents context pollution|
|Flow A                |Downward context assembly: LTM/RRC/STM/EB → WM at inference start                                                                                           |
|Flow B                |Upward context commit: WM outputs → EB → STM → LTM via OER/HLC                                                                                              |
|Flow C                |Cross-session serialization: WM state → LTM session boundary block at session end                                                                           |
|Flow D                |Cross-agent context handoff: Agent_i WM summary → Agent_j EB for DCoT chain                                                                                 |
|λ_atf                 |Anchor Token Frequency — ATI injection rate parameter; higher = more frequent re-injection                                                                  |
|Session boundary block|LTM block type committing WM state at session end; enables cross-session continuity                                                                         |
|Context poisoning     |Contamination of the context window with incorrect, outdated, or adversarially crafted information                                                          |

-----

## Appendix B: Proof Index (P1–P61)
...
|P58  |**v6.4**|**§25.3**|**CAE priority ordering maximizes Q(wm) per token budget (information-theoretic sketch)**|
|P59  |**v6.4**|**§26.1**|**DACS: 90.0–98.4% steering accuracy vs. 21.0–60.0% flat-context baseline (Patel 2026)** |
|P60  |**v6.4**|**§26.2**|**KVPSP: KV compute reduction = d_shared / (2×d_total); 30% for 60% shared prefix**      |
|P61  |**v6.4**|**§25.5**|**ATI: omission constraint recall ≥ 1 - exp(-λ_atf × D × w_anchor/W_budget)**            |
...
*Citation: Cybopsec Research (2026). DARM-ANN v6.4: Distributed Agentic Recursive Memory Networks. Working White Paper, May 2026. <https://cybopsec.github.io/darm-ann>*

*Version 6.4 supersedes v6.3 (May 2026). All prior versions archived at <https://github.com/cybopsec/darm-ann>*

```bibtex
@techreport{cybopsec2026darmannv64,
  title={DARM-ANN v6.4: Distributed Agentic Recursive Memory Networks},
  ...
  note={Version 6.4 — Context Bridge and Engineering Edition}
}
```